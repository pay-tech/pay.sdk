...
package com.tec.pay.android.base.frame;
 ...
 * usually a Presenter should not get any data from View: so calls like view.getUserId() should not
 * be done. Rather write a method in your Presenter that takes the user id as parameter like this:
 * {@code void doSomething(int userId){ // do something ...
 *
...
public class MvpBasePresenter<V extends MvpView> implements MvpPresenter<V> {
  ...
  /**
   * An Action that is executed to interact with the view. Usually a Presenter should not get any
   * data from View: so calls like view.getUserId() should not be done. Rather write a method in
   * your Presenter that takes the user id as parameter like this: {@code void doSomething(int
   * userId){ // do something ...
   *
   * ifViewAttached( view -> view.showSuccessful()) }
   *
  ...
  /**
   * Executes the passed Action only if the View is attached. If no View is attached, either an
   * exception is thrown (if parameter exceptionIfViewNotAttached is true) or the action is just not
   * executed (no exception thrown). Note that if no view is attached, this will not re-execute the
   * given action if the View gets re-attached.
   *
   * @param exceptionIfViewNotAttached true, if an exception should be thrown if no view is attached
   * while trying to execute the action. false, if no exception should be thrown (the action will
   * not be executed since no view is attached)
   * @param action The {@link ViewAction} that will be executed if a view is attached. This is where
