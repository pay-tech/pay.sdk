 ...
 */
package com.tec.pay.android.task;
...
public class Task<TResult> {
  ...
  /**
   * An {@link Executor} that executes tasks in the current thread unless the stack runs too deep,
   * at which point it will delegate to {@link Task#BACKGROUND_EXECUTOR} in order to trim the
   * stack.
   */
   ...
   * anything time consuming or complex it is a good idea to fire off a {@code Task} to handle the
   * exception.
   *
   ...
   *
   * If you want to cancel the resulting Task throw a {@link CancellationException} from the
   * callable.
   */
   ...
   * in a faulted state, where its exception will resolve to that {@link Exception} if a single task
   * fails or an {@link AggregateException} of all the {@link Exception}s if multiple tasks fail.
   * <p/>
  ...
  /**
   * Runs a continuation when a task completes successfully, forwarding along {@link Exception} or
   * cancellation.
   */
  ...
  /**
   * Runs a continuation when a task completes successfully, forwarding along {@link Exception}s or
   * cancellation.
   */
  public <TContinuationResult> Task<TContinuationResult> onSuccess(
