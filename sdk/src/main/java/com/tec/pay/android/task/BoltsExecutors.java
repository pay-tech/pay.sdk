...
/* package */ final class BoltsExecutors {
   ...
   * at which point it will delegate to {@link BoltsExecutors#background} in order to trim the
   * stack.
   */
  /* package */
  ...
  /**
   * An {@link Executor} that runs a runnable inline (rather than scheduling it on a thread pool) as
   * long as the recursion depth is less than MAX_DEPTH. If the executor has recursed too deeply, it
   * will instead delegate to the {@link Task#BACKGROUND_EXECUTOR} in order to trim the stack.
   */
